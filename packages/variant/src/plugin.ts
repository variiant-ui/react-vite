...
import {
  handleVariantTweakApplyRequest,
  handleVariantTweakCatalogRequest,
  variantTweakApplyRoute,
  variantTweakCatalogRoute,
} from "./plugin-tweaks";
...
export type { VariantRegistryEntry, VariantTargetEntry } from "./bundler-core";
...
export type VariantDevMiddlewareOptions = {
  projectRoot?: string;
  variantsDir?: string;
  agentRefresh?: "hmr" | "full-reload";
  onRefresh?: (changedFiles: string[], refreshMode: "hmr" | "full-reload") => Promise<void>;
};

export type VariantDevMiddleware = (
  req: IncomingMessage,
  res: ServerResponse,
  next: () => void,
) => void;
...
export function createVariantDevMiddleware(options: VariantDevMiddlewareOptions = {}): VariantDevMiddleware {
  ...
  return (req, res, next) => {
    ...
    if (pathname === variantTweakCatalogRoute && req.method === "POST") {
      void handleVariantTweakCatalogRequest(req, res, {
        configuredVariantsDir: options.variantsDir,
    ...
    if (pathname === variantTweakApplyRoute && req.method === "POST") {
      void handleVariantTweakApplyRequest(req, res, {
        configuredVariantsDir: options.variantsDir,
        onComplete: async (changedFiles) => {
          await options.onRefresh?.(changedFiles, "hmr");
        },
        parseJsonRequestBody,
        projectRoot,
        sessionToken: agentSessionToken,
  ...
  return {
    ...
    load(id) {
      ...
      if (this.meta.watchMode) {
        return buildDevelopmentProxyModule(entry, resolveDevelopmentRuntimeImport());
      }
